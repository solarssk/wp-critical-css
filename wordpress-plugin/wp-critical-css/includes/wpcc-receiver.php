<?php
/**
 * REST endpoint that receives generated critical CSS from the self-hosted critical-css-service and stores it per-post. Part of the WP Critical CSS plugin - loaded from the main wp-critical-css.php file, one of four includes (trigger / receiver / inject / shared) - see the repo's README for the full architecture.
 *
 * Stored per-post (not per-template) - useful if your page builder emits a separate physical CSS file per post, since the critical subset then differs per post too, not just per template.
 *
 * Auth is a shared secret in a custom header, not WP's cookie/nonce auth - this is a machine-to-machine call from a container with no WP user account. permission_callback is intentionally public; the real check happens inside the handler via hash_equals() (timing-safe).
 *
...
if ( ! function_exists( 'wpcc_receiver_fixed_window_limited' ) ) {
	/**
	 * A fixed-window counter under an arbitrary transient key, backed by WP's transient API (options table if no external object cache is configured, the real cache otherwise) - no new dependency, and correct at the traffic volumes this route actually sees (at most one call per post save, or one per WPCC_SWEEP_DELAY_MS-spaced sweep step from the generator).
	 *
	 * The window's own start time is stored IN the value (`"<window_start>:<count>"`), and window expiry is computed here by comparing that timestamp to time() - not left to the transient's own TTL. set_transient() unconditionally resets its entry's expiry on every write, so relying on that TTL to signal "window expired" would mean any caller making a request more often than the window apart keeps pushing the window out indefinitely and the counter never resets - on a real site (especially one backed by Redis/Memcached, where this is most visible) that can eventually rate-limit the generator's own legitimate sitemap-sweep deliveries once enough of them land inside one never-expiring window, silently leaving pages without critical CSS. The transient's own TTL below is set generously past the real window purely so WordPress eventually garbage-collects the row - it no longer defines the window boundary itself.
	 *
	 * Known tradeoff, not fixed here: the get_transient()+set_transient() pair below is a non-atomic read-modify-write, so concurrent requests against the same key can race and undercount by roughly the size of the PHP worker pool handling them. This is a defense-in-depth control behind the shared secret, not the primary security boundary, and a correct fix needs a backend-specific atomic primitive (this route has to work whether transients happen to be backed by the options table or by a persistent object cache) - accepted rather than papered over with a fix that would only be correct for one of those two backends.
	 *
...
if ( ! function_exists( 'wpcc_receiver_auth_rate_limited' ) ) {
	/**
	 * Throttles repeated FAILED secret attempts, keyed by caller IP - brute-force protection. Deliberately NOT applied to successful (valid-secret) requests: this route sits behind a reverse proxy/CDN on plenty of real deployments, and unless that layer restores the true client IP into REMOTE_ADDR, every caller - including the generator's own legitimate traffic - shares one apparent address. Sharing a bucket between "public internet noise probing with wrong secrets" and "the generator's own valid deliveries" would let that noise 429 the generator's real work. Keyed only on failure, this bucket now only ever contains actual brute-force attempts.
	 *
...
if ( ! function_exists( 'wpcc_receiver_write_rate_limited' ) ) {
	/**
	 * Bounds total write volume from a VALID secret - the DB-bloat-DoS control. Deliberately a single global bucket, not per-IP: there is exactly one valid WPCC_SHARED_SECRET for the whole site by design (see wpcc-trigger.php/wp-config.php), so "per caller" was never the right shape for this cap in the first place - a global counter maps directly onto "how fast can the one valid secret be used to write", and is immune to REMOTE_ADDR being proxy-shared/ambiguous, unlike a per-IP cap would be.
	 *
	 * @return bool True if authenticated write volume is currently over the limit and this request should be rejected.
	 */
...
if ( ! function_exists( 'wpcc_receiver_auth_error' ) ) {
	/**
	 * Validates the shared secret and, on failure, applies the IP-keyed brute-force throttle - collapsed into one call so wpcc_receive() doesn't carry this branching itself.
	 *
	 * @param WP_REST_Request $request
	 * @return WP_REST_Response|null A response to return immediately, or null if the secret is valid.
	 */
	function wpcc_receiver_auth_error( WP_REST_Request $request ) { // NOSONAR php:S100 - WordPress Coding Standards mandate snake_case; matches every other function in this directory
		...
		// IP-keyed brute-force throttle - only ever counts actual failed attempts (see wpcc_receiver_auth_rate_limited()'s own doc comment for why this can't share a bucket with the write-volume cap wpcc_receive() applies separately, after authentication).
...
if ( ! function_exists( 'wpcc_receiver_post_is_eligible' ) ) {
	/**
	 * Mirrors the whitelist wpcc-trigger.php already applies before ever scheduling generation (post/page, published only) - without this, url_to_postid()'s ?p=N/?page_id=N/?attachment_id=N fallback lets any resolvable post ID be written to, including attachments, drafts, and other post types this plugin was never meant to touch.
	 *
	 * @param int $post_id
	 * @return bool
	 */
...
if ( ! function_exists( 'wpcc_receiver_store_front_page_css' ) ) {
	/**
	 * The homepage isn't a post_id wpcc_receiver_store_css() can key on - see the doc comment on the front-page check in wpcc_receive() for why. Stored as options instead, mirroring WP Rocket's own approach to the exact same generator-can't-resolve-the-homepage problem (its critical-CSS feature writes a dedicated front_page.css file rather than trying to attach the homepage's CSS to any particular post). `autoload => false`: this is only ever read on the one front-page request, not worth carrying into every request's autoloaded options blob the way small, always-needed options are.
	 *
	 * @param string $css_mobile
	 * @param string $css_desktop
	 * @return void
...
function wpcc_receive( WP_REST_Request $request ) { // NOSONAR php:S1142 - each early return is a distinct validation failure with its own HTTP status/message; collapsing them into one exit point would either merge unrelated error codes or need a flag-plus-nesting rewrite, both worse than this guard-clause sequence
	...
	// Reached only with a valid secret - global (not per-IP) write-volume cap, see wpcc_receiver_write_rate_limited()'s own doc comment.
