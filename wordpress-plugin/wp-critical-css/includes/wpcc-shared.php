...
if ( ! function_exists( 'wpcc_css_string_end' ) ) {
	/**
	 * `$css[$i]` is an opening quote character - returns the index just
	 * past its matching closing quote (respecting backslash-escapes), or
	 * strlen($css) if the string is never closed.
	 *
	 * @param string $css
	 * @param int    $length
	 * @param int    $i
...
if ( ! function_exists( 'wpcc_css_comment_end' ) ) {
	/**
	 * `$css[$i]`/`$css[$i+1]` are the opening `/` `*` of a CSS comment -
	 * returns the index just past its closing `*` `/`, or strlen($css) if
...
if ( ! function_exists( 'wpcc_css_is_import_at' ) ) {
	/**
	 * Whether `$css[$i]` starts a real `@import` at-rule - only reachable
	 * when the caller has already confirmed position $i is outside any
	 * string/comment, so this only needs to check the token itself and a
	 * word-boundary after it (so e.g. `@importantx` isn't mismatched).
	 *
	 * @param string $css
	 * @param int    $i
	 * @return bool
...
if ( ! function_exists( 'wpcc_css_import_statement_end' ) ) {
	/**
	 * `$i` is the index right after a confirmed `@import` token - returns
	 * the index just past this statement's own closing `;` (tracking
	 * quote state through the statement's own value, so a `;` inside e.g.
	 * `@import "foo;bar.css";` doesn't end the strip early), or
	 * strlen($css) if it's never terminated.
	 *
	 * @param string $css
	 * @param int    $length
	 * @param int    $i
...
if ( ! function_exists( 'wpcc_strip_import_statements' ) ) {
	/**
	 * A character-scanner, not a regex - two rounds of regex boundary
	 * heuristics here (first "preceded by `;{}` or whitespace", then still
	 * broken because whitespace INSIDE a quoted string is also whitespace)
	 * both got corrected by finding real CSS content they corrupted, e.g.
	 * `content:"hello @import world"`. A regex fundamentally cannot tell
	 * "inside an unclosed string" from "at the top level" without tracking
	 * quote state character by character, so that's what this does
	 * instead, via the small helpers above: `@import` is only ever treated
	 * as a real at-rule when the scanner is NOT currently inside a
	 * string/comment, which is both the necessary and the sufficient
	 * condition - no boundary-character guessing needed.
	 *
	 * @param string $css
	 * @return string
	 */
...
if ( ! function_exists( 'wpcc_sanitize_css' ) ) {
	 ...
	 *   wpcc_strip_import_statements() above for why this is a real
	 *   scanner, not a regex.
	 * Applied on both write (wpcc-receiver.php) and read (wpcc-inject.php)
	 * - two independent call sites, neither trusts the other.
	 *
	 * `url(...)` is deliberately left untouched: real critical CSS
	 * legitimately contains it (hero background-images, @font-face src),
	 * and stripping it would break correctly-generated output for the one
	 * thing this plugin exists to do - see docs/SECURITY-CONTROLS.md for
	 * the reasoning behind that tradeoff.
	 *
	 * @param string $css
	 * @return string
	 */
