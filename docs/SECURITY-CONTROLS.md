...
| Container image CVEs | every version tag, weekly (Monday 04:00 UTC, re-scanning the default branch even with no new tag), or manual dispatch (scan-only on a plain branch) | `.github/workflows/publish-container.yml` - Trivy; a full SARIF report (CRITICAL/HIGH/MEDIUM) is always uploaded to the Security tab, and a hard gate blocks the push on any fixable CRITICAL finding |
