...
| A Chrome sandbox escape/renderer compromise persisting a backdoor into the container's own filesystem (the container already runs with the elevated `SYS_ADMIN` capability Chrome's sandbox needs) | `read_only: true` root filesystem, with `tmpfs` mounts only for the specific paths Chrome's own scratch/profile directory needs at runtime - verified empirically against a real render, not assumed | `docker-compose.example.yml` |
| Default Docker capability set (`NET_RAW`, `SETUID`, etc.) sitting unused alongside the deliberately-added `SYS_ADMIN` | `cap_drop: ["ALL"]` alongside `cap_add: ["SYS_ADMIN"]` - verified empirically that a real render still succeeds with every other default capability dropped | `docker-compose.example.yml` |
...
| No health signal when the image is run outside the example compose file (a plain `docker run`, or an orchestrator that doesn't carry that file's `healthcheck:` block forward) - silently defeats anything tied to container health, including this image's own `restart: always` policy | `HEALTHCHECK` baked directly into the image, not left as an opt-in compose stanza | `service/Dockerfile` |
